 ...
 */
#define MAX_GTK_NEW_TERM 1


/*
 * An array of "term_data" structures, one for each "sub-window"
 */
static term_data data[MAX_GTK_NEW_TERM];
...
errr init_gtk(int argc, char **argv)
{
	...
	g_signal_connect(gtk_window, "delete-event", G_CALLBACK (quit_gtk), NULL);
	g_signal_connect(gtk_window, "destroy",  G_CALLBACK (quit_gtk), NULL);
